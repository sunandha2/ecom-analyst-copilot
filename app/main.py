...
groq_api_key = st.secrets.get("GROQ_API_KEY", os.getenv("GROQ_API_KEY")) if hasattr(st, 'secrets') else os.getenv("GROQ_API_KEY")
...
def load_full_metrics():
    return pd.read_csv('outputs/weekly_metrics.csv')
...
with st.sidebar:
    ...
    st.markdown(f"- ⚠️ {report['anomaly_flags'].sum()} anomalies detected")
    st.markdown("---")
